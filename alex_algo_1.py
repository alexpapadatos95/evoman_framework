...
def crossover(pop):
    ...
    for p in range(0,pop.shape[0], 2):
        ...
        for f in range(0,n_offspring):
            ...
            # mutation
            N_tau = np.random.normal(0, 0.2)
            N_tau_i = np.random.normal(0, 0.05)
            sigma_i = math.sqrt(0.05)
            
            sigma_i = sigma_i * math.exp(N_tau + N_tau_i) 

            for i in range(0,len(offspring[f])):
                if sigma_i <= mutation:
                    normal_i = np.random.normal(0, 1)
                    offspring[f][i] =  offspring[f][i] + sigma_i * normal_i
                    sigma_i = sigma_i * math.exp(N_tau + normal_i)
